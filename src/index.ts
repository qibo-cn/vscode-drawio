				...
				if (data.project_info) {
					...
					vscode.window.showSaveDialog(options).then(fileUri => {
						if (fileUri && fileUri) {
							...
							fs.mkdir(path.join(fileUri.path + '/' + proj_desc_info.project_name), (error) => {
								if (error) {
									log.error(error);
								} else {
									fs.mkdir(path.resolve(fileUri.path + proj_desc_info.project_name + '/model'), (error) => {
										if (error) {
											log.error(error);
										} else {
											fs.writeFile(path.resolve(fileUri.path + proj_desc_info.project_name + '/model' + '/package.json'), data, (error) => {
												if (error) {
													log.error(error);
												} else {
